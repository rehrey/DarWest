...
@section('contents')
                                                ...
                                                <div class="card card-custom gutter-b bg-transparent shadow-none border-0">
                                                    <div class="card-header align-items-center  border-bottom-dark px-0">
                                                        <div class="card-title mb-0">
                                                            <h3 class="card-label mb-0 font-weight-bold text-body">Payment Method
                                                            </h3>
                                                        </div>
                                                                                ...
                                                                                <div class="card-toolbar text-right">
                                                                                    ...
                                                                                    <div class="dropdown-menu dropdown-menu-right"
                                                                                        ...
                                                                                        <a class="dropdown-item"
																							...
																							data-target="#edit_paymentmethod{{ $paymentmethod->id }}">Edit</a>
                                                                                        <a class="dropdown-item"
                                                                                            title="Delete"
                                                                                            href="#" data-toggle="modal"
																							data-target="#deletepaymentmethod{{ $paymentmethod->id }}">Delete</a>
                                                                                    </div>
                                                                                </div>

	{{-- ----Editing Medicine form-------- --}}

    <div class="modal fade" id="edit_paymentmethod{{ $paymentmethod->id }}" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog modal-dialog-scrollable" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					action="{{ URL::to('upd_paymentmethod') }}">
					@csrf
						<div class="row">
							<div class="col-12">
								<input type="text" value="{{ \base64_encode($paymentmethod->id) }}"
                                hidden name="id" />
								<div class="form-group">
									<label class="text-dark">Payment Method Name</label>
									<input type="text" class="form-control" placeholder="Medicine Category Name"
									name="name" value="{{ $paymentmethod->name }}" >
	...
	{{-- ----Delete Payment Method form-------- --}}

    <div class="modal fade" id="deletepaymentmethod{{ $paymentmethod->id }}" tabindex="-1" role="dialog"
		aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
		<div class="modal-dialog modal-dialog-scrollable" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="exampleModalScrollableTitle">
						Delete Payment Method </h5>
					<button type="button"
						class="close rounded-pill btn btn-sm btn-icon btn-light btn-hover-primary m-0"
						data-dismiss="modal" aria-label="Close">
						<svg width="20px" height="20px" viewBox="0 0 16 16" class="bi bi-x" fill="currentColor"
					...
					@csrf
						<div class="row">
							<div class="col-12">
								...
								<div class="form-group">
									Are You sure you want to delete {{ $paymentmethod->name }} Payment Method?
								</div>
							...
							<div class="modal-footer">
								...
								<a href="{{ URL::to('delete_paymentmethod',['id'=>\base64_encode($paymentmethod->id)]) }}"  class="btn btn-primary" type="submit">Submit</a>
        ...
        <div id="kt_notes_panel" class="offcanvas offcanvas-right kt-color-panel p-5">
            ...
            <span id="check_paymentmethod_span" style="color:red"></span>

            <div class="offcanvas-header d-flex align-items-center justify-content-between pb-3">
                <h4 class="font-size-h4 font-weight-bold m-0">Add Payment Method
                </h4>
            ...
            <form id="myform" method="post" action="{{ URL::to('add_paymentmethod') }}">
                @csrf
                <div class="row">
                    <div class="col-12">
                        ...
                        <div class="form-group">
                            <label class="text-dark">Payment Method Name</label>
                            <input type="text" name="name" class="form-control check_paymentmethod" placeholder="Enter Medicine paymentmethod" id="check_paymentmethod">
                           
                        </div>

                    </div>
                </div>
                <button type="submit" class="btn btn-primary" id="add_btn_paymentmethod">Submit</button>
            </form>
    ...
    $(document).on('keyup','.check_paymentmethod',function()
    {
            var paymentmethod_name=$(this).val();
            // var div=$(this).parent();
            // Ajax to load pump price
            console.log(paymentmethod_name);
            $.ajax({type:'get', url:'{!! URL::to('check_paymentmethod') !!}', //URL to load the data
                data:{'name': paymentmethod_name}, 
                //processData: false,//contentType: false,
                success:function(data){
                    console.log(data);
                    if(data>=1){
                        var span_sms=document.getElementById('check_paymentmethod_span');
                        span_sms.innerHTML="* Payment Method already exist!";
                        document.getElementById("add_btn_paymentmethod").disabled = true;
                    }
                    else{
                        var span_sms=document.getElementById('check_paymentmethod_span');
                        span_sms.innerHTML=" ";
                        document.getElementById("add_btn_paymentmethod").disabled = false;
                    }
                },
                error:function(){
                },
